...
{
    public static TestRouteSolution Solve(
        IReadOnlyList<TestRouteNode> nodes,
        IReadOnlyList<TestRouteEdge> edges,
        int startId,
        int goalId)
    {
        int n = nodes.Count;
        var dist = new float[n];
        var previousEdge = new TestRouteEdge[n];
        ...
        var adjacency = new List<TestRouteEdge>[n];
        for (int i = 0; i < n; i++) adjacency[i] = new List<TestRouteEdge>();
        ...
        if (!visited[goalId])
            return new TestRouteSolution { Reachable = false };

        var resultEdges = new List<TestRouteEdge>();
        int cur = goalId;
        while (cur != startId)
        {
